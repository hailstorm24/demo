document.addEventListener('DOMContentLoaded', () => {
    let wordChecks = {};
    let originalText = "Paste text here..."
    ...
    const rescanButton = document.querySelector('.btn-rescan');
    ...
    async function fetchUpdatedWordChecks() {
        try {
            ...
            const response = await fetch('http://127.0.0.1:5000/adjectives', {
                method: 'POST',
                headers: {
    ...
    if (clearButton) {
        ...
        function clearText() {
            if (contentEditable.innerHTML === originalText) {
                contentEditable.innerHTML = '';
            }
        }
        ...
        function restoreText() {
            if (contentEditable.innerHTML === '') {
                contentEditable.innerHTML = originalText;
            }
        }

    function updateText() { 
        originalText = contentEditable.innerHTML
    ...
    // Event listener to clear text on first edit
    contentEditable.addEventListener('focus', clearText, { once: true });

    // Event listener to restore text if empty
    contentEditable.addEventListener('blur', restoreText);

    contentEditable.addEventListener('input', updateText);
